...
The approved CLI is specified in [the CLI contract](docs/CLI-CONTRACT.md). Parent 1 is complete: the canonical TrackSequence, Source pipeline, authentication behavior, and top-level command shell are in place. Parent 2 is in progress: new-public Build and owned empty-target Build are implemented, while `add`, `verify`, and `export` remain fail closed; Parent 3 `search` and `inspect` do the same.
